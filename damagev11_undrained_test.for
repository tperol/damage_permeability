...
c     update the pore fluid pressure for next time step
      statenew(ki,ntens+10)= p_0 - (K_u - K) * (estrain(1) 
...
c     test: keep pore fluid pressure from xitime
c           to evaluate constitutive response     

c     update the pore fluid pressure
c      d_p = - (estrain(1) + estrain(2) + estrain(3))
c     1      /  (biot / (K_u - K_d  )  )
...
C----
      do i=1,3
c     CHANGES FOR UNDRAINED DEFORMATION            
       stress(i)=(2.0*g*estrain(i) + temp1*sm)/temp0 - biot*(p_f)
      enddo
C----
      do i=1,3
...
c     CHANGES FOR UNDRAINED DEFORMATION            
       ...
       stress(i) = stress(i)/tempfacs - biot*(p_f)
      enddo
