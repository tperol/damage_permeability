...
C----+	  else
C----+	    statenew(ki,ntens+1) = yold(1)
C----+	  endif

      if ((sm.le.test1).AND.(yold(1).eq.d0)) then
         xk=0.0
      else
		   if (sm.lt.test2) then
			  xk=sqrt(pi*a)*(aa*sm+bb*se)
		   elseif (sm.ge.test2) then
...
C----+-----------------------------------------------------------------+
     ...
     +    (1.0/(1.0/d0**(2.0/3.0)-d**(2.0/3.0)))
      ...
      if ((sm.le.test1).AND.(y(1).eq.d0)) then
         xk=0.0
      else
		   if (sm.lt.test2) then
			  xk=sqrt(pi*a)*(aa*sm+bb*se)
...
C----+C----+-----------------------------------------------------------------+
      ...
      subroutine upstress(tempdamage)
      include 'vaba_param.inc'
      common/ceramic/den,xnu,es,xkic,d0,
     +        xf,alpha,beta,gammat,xfric,vm,xlodot,xxm,flag
      common/state/strain(6),xstrain(6),estrain(6),em,ex
      ...
      if ((em.le.test1).AND.(tempdamage.EQ.d0)) then
       regime = 1.0
       call hooke
      else
		   if (em.lt.test2) then
